...
- **Fixed voice origin is the drift baseline — not the anchor, not the neighbors.** The constitution is the *fixed* bar. Establish a **fixed origin**: the constitution, plus the concept's voice & tone section, plus the *earliest* approved (`[X]`) chapters. Grade voice against that origin. `book/style-anchor.md` is only a derived continuity aid — `/authorkit.write` regenerates it from that same origin, but a stale or hand-edited anchor can lag, so never treat the anchor (or the immediately adjacent chapters) as the standard for what "good" sounds like. A chapter that matches its recently-drifted neighbors but has slipped from the origin **is** drifting, and that is a finding — not a defense.
...
- **Calibration sanity check**: re-read the earliest approved chapter against the *current* constitution and style anchor. If that origin chapter would no longer pass today's bar, the **bar has drifted** — e.g. a stale or hand-edited style anchor, or a voice evolution that was never recorded in the constitution. Flag it and recommend re-grounding the anchor via `/authorkit.write` (which regenerates it from the origin).
