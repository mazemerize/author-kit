...
- **Severity mapping** (density- and trend-based — this is what makes a shape *over budget*, i.e. gating-eligible). Each ledger entry carries a **`Budget:`** — instances per chapter, per 1,000 words for texture/density shapes, or **0** for flag-on-sight forms. Seeded entries inherit it from the seed catalog's budget table at bootstrap; discovered entries default to **3 per chapter**, counted **per 1,000 words (0.75/1k)** in chapters over ~4,000 words so a long chapter gets no extra allowance and a short one is not over-gated:
  ...
  - **Configurable thresholds**: the three density thresholds below default to 3 but may be tuned per book in `book/book.toml`'s `[review]` table — `cluster_min_shapes` (default 3), `tic_load_threshold` (default 3.0), `persistence_chapters` (default 3). Read the table once at the start of Step B when the file exists; a missing file, table, or key means the default. Lower values tighten the gate; values below 1 (or 1.0) are treated as the default.
  ...
  - **Tic-load index (chapter-wide compounding)**: compute `load = Σ (instances ÷ budget)` over all **active, non-waived, non-zero-budget** shapes in this draft (normalize per-1,000-word budgets to this chapter's length). **Load at/above `tic_load_threshold` (default 3.0) → Critical and gating** even when no single shape is over budget — many below-budget tics compounding is the same density signal as one rampant shape. Report the computed load, the threshold in effect, and its top contributors in the finding.
