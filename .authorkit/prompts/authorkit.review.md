...
- **Fixed voice origin is the drift baseline — not the anchor, not the neighbors.** The constitution is the *fixed* bar. `book/style-anchor.md` is a *rolling* aid that is regenerated from recently approved chapters, so it can quietly track drift instead of resisting it. Never treat the style anchor — or the immediately adjacent chapters — as the standard for what "good" sounds like. Establish a **fixed origin**: the constitution, plus the concept's voice & tone section, plus the *earliest* approved (`[X]`) chapters. Grade voice against that origin. A chapter that matches its recently-drifted neighbors but has slipped from the origin **is** drifting, and that is a finding — not a defense.
...
3. **Load core context** (used by both modes). Load the fixed references *first* and hold them as the bar **before** reading the chapter under review, so the standard is set in advance rather than calibrated to the prose in front of you:
   ...
   - `STYLE_ANCHOR` at `BOOK_DIR/style-anchor.md` — cadence, diction/register, imagery density, dialogue profile, drift flags. Use it as a continuity aid, but remember it is regenerated from recent approvals: where it disagrees with the constitution or the origin, the constitution and origin win.
...
- **Voice fidelity vs origin**: compare the chapter's voice — POV, narrative distance, sentence rhythm, diction/register, imagery — against the **fixed origin** (constitution + concept voice/tone + the earliest approved chapters), not merely against the style anchor or the previous chapter. Flag drift from the origin even when the chapter reads as locally consistent with its neighbors. Distinguish *unsanctioned* drift (a finding, at least Important; Critical if it is also a constitution violation) from *constitution-sanctioned* evolution (not a finding). Quote the specific lines that diverge.
...
| Voice Fidelity (vs Origin) | [A/B/C/D] | [Drift from constitution + earliest approved chapters; note if sanctioned] |
...
Per-chapter checks catch absolute violations but miss *gradual* drift where every chapter is individually defensible yet the book has slid a long way from where it started. Establish the **fixed origin** (constitution + concept voice/tone + the earliest approved chapters) and trace the *direction* of change across the chapter sequence, not just per-chapter compliance:
...
- **Origin jump test**: compare the latest chapters directly against the earliest approved chapters. If a reader started at the origin and jumped to the latest chapter, would it read as the same book, same narrator, same voice? Quote the divergence.
- **Calibration sanity check**: re-read the earliest approved chapter against the *current* constitution and style anchor. If that origin chapter would no longer pass today's bar, the **bar itself has drifted** (the style anchor has tracked the slippage rather than resisting it). Say so explicitly and recommend re-grounding the style anchor from the constitution + origin (via `/authorkit.write`) rather than from recent chapters.
...
| Metric | Origin (earliest [X]) | Latest | Direction | Verdict |
|--------|----------------------|--------|-----------|---------|
...
*Bar calibration*: [does the earliest approved chapter still pass today's constitution + style anchor? if not, the anchor has drifted — recommend re-grounding]
...
- **Anchor to the origin, not the neighbors**: grade voice against the fixed origin (constitution + concept voice/tone + earliest approved chapters). "Consistent with the last chapter" never establishes that something is correct — recent chapters may already have drifted.
