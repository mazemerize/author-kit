...
- **Gating & convergence (carry-over rule — this is what makes the loop terminate).** Blind Step A always runs and always *discovers* (it keeps the ledger learning), but discovery must not re-open the gate every cycle. Distinguish the chapter's **gating set** from mere discovery:
  ...
  - **Re-review of a chapter** (a prior `chapters/NN/review.md` exists): read the prior review's `**Gating Shapes**:` line. The gate set is exactly the shapes **at/above the density budget in this draft** — which is the **carry-over set** (prior gating shapes still over budget) **plus regressions** (a shape a revise introduced or worsened past budget, which the prior review did not gate; these still gate, so a revise can never trade one tic for another and pass). A carry-over shape drops off the gate the moment revise brings it within budget. It is the budget threshold — not an exclusion list — that keeps the gate finite: the blind pass may name new shapes, but only ones that are actually rampant (≥ budget) can gate.
  ...
  - **Converged-with-residual → PASS Pass 2**: when no shape is over budget (every previously-gating shape is now within budget and no regression pushed a new one over it), Pass 2 **passes**, even if Step A surfaced new below-budget residual shapes this cycle. Note it in the review ("converged-with-residual: <residual shapes seeded forward>"). For a fixed draft the gate set is exactly its over-budget shapes — stable and non-growing across repeated reviews — and each applied revise strictly shrinks it toward empty: a guaranteed fixed point.
