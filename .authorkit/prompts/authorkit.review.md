...
1. **Setup**: Run `{{SCRIPT_CHECK_PREREQ}}` from repo root and parse `BOOK_DIR`, `BOOK_LANGUAGE`, `STYLE_ANCHOR`, and `AVAILABLE_DOCS`. All paths must be absolute. Abort with a clear error if required files are missing. Resolve `BOOK_LANGUAGE` (absent ⇒ `en-US`) per the shared guardrails' *Language Protocol*: findings prose is written in the book's language, quoted draft text is kept verbatim, and ids, severity labels, verdict labels, and the `**Gating Shapes**:` line stay English.
...
- **Language** (see the shared guardrails' *Language Protocol*): cadence, register, and dialogue behaviour are judged by the conventions of the book's own language against the origin written in it — never against English rhythm or punctuation expectations. A chapter drafted in a different language than `BOOK_LANGUAGE` and the origin is a Critical Pass 1 finding: report it and let the author decide, rather than translating it yourself.
...
- **Bootstrap (first run only)**: if `book/tic-ledger.md` does not exist, create it from `.authorkit/templates/tic-ledger-template.md`, seeding entries with `Status: seed` from **two sources — the shape catalog and the book's language pack** (see the catalog's *Language Scope & Packs* and *How to Apply*). Seeds are hypotheses; the steps below confirm or retire them.
  - **Shapes**: `.authorkit/prompts/_shared/literary-tic-catalog.md` — high-signal patterns 7, 13, 21, 22, 23, 24, 29, 33, 35, 36, 41 and the zero-budget forms of 3, 14, 15, 21, 23, 24, 29, 35, 39. Record `Lang: universal`. Never seed `Volatility: high` entries (28) — those enter only via blind discovery.
  - **Realizations**: `.authorkit/prompts/_shared/tic-catalog-<lang>.md`, resolved from `BOOK_LANGUAGE` on the **primary subtag** (`en-US`/`English` → `en`; `fr-CA`/`fr-FR` → `fr`; absent ⇒ `en-US`). Attach each seeded shape's exact strings and examples from the pack, and seed the pack's own language-only entries (`FR-nn` and the like) with `Lang: <subtag>` and the pack's budgets. **This applies to English exactly as it does to every other language** — English realizations live in `tic-catalog-en.md`, not in the shape catalog. A shape whose only content is its realization (29, and the zero-budget string forms of 3, 14, 15, 21, 23, 24, 35) seeds **only** when the pack supplies it.
  - **No pack for the book's language**: seed the shapes alone and say so in the review — the first chapters run with no language-specific string sweep while blind discovery builds the ledger. Never substitute another language's pack; its strings cannot occur in the prose and a permanently-zero entry distorts the tic-load mean.
...
- **Severity mapping** (density- and trend-based — this is what makes a shape *over budget*, i.e. gating-eligible). Each ledger entry carries a **`Budget:`** — instances per chapter, per 1,000 words for texture/density shapes, or **0** for flag-on-sight forms. Seeded entries inherit it from the seed catalog's budget table at bootstrap; discovered entries default to **3 per chapter**, counted **per 1,000 words (0.75/1k)** in chapters over ~4,000 words so a long chapter gets no extra allowance and a short one is not over-gated:
  ...
  - **Tic-load index (chapter-wide compounding)**: compute `load = Σ (instances ÷ budget) ÷ N` — the **mean budget utilization** — over all **active, non-waived, non-zero-budget** ledger entries, each evaluated against this draft (normalize per-1,000-word budgets to this chapter's length). **`N` counts every such entry, including those with zero instances this chapter** — it is the size of the tracked active set, *not* the number of shapes that happen to appear. This is load-bearing: excluding the zeros shrinks `N`, inflates the mean, and re-creates the very coupling this index exists to remove, since the set of shapes *present* varies chapter to chapter. It would also duplicate the severity mapping's job — intensity among present shapes is already per-shape gating; this index measures how much of the tracked repertoire is simultaneously crowding its ceilings. Every entry has an effective budget even when it omits the `Budget:` field: fall back to the severity mapping's default (3 per chapter, or 0.75/1,000 words in chapters over ~4,000 words), so `N` is always well-defined. An instance that two cross-referenced entries both claim counts **once**, attributed to the entry with the tighter budget. **Load at/above `tic_load_mean_threshold` (default 0.75) → Critical and gating** even when no single shape is over budget — many tics all crowding their ceilings is the same density signal as one rampant shape. Report the computed mean, `N`, the threshold in effect, and the top contributors in the finding.
    ...
    - **Budgets are calibrated against the origin too.** A budget — especially a per-1,000-word rate — set at or below the origin's own measured rate for that shape is **mis-set by definition**: it taxes every correctly-written chapter with a contribution near 1.0 that no revision can remove, because writing like the origin is the standard. The per-shape origin counts the canary pre-check already produces are what reveal this (the targeted sweep covers *this* draft, not the origin): whenever those counts put the origin at/above a shape's own budget, flag that budget for correction — raise it clear of the origin's rate — rather than carrying it. Record the corrected budget on the ledger entry so the fix persists. Do not instead drop the shape from the index — a chapter genuinely running several times the origin's rate must still register.
