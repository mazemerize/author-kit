...
- **Bootstrap (first run only)**: if `book/tic-ledger.md` does not exist, create it from `.authorkit/templates/tic-ledger-template.md`, seeding entries with `Status: seed` from the shipped seed catalog's high-signal patterns (`.authorkit/prompts/_shared/literary-tic-catalog.md` — patterns 7, 13, 21, 22, 23, 24, 29, 33, 35, 36, 41 and its zero-budget forms; never `Volatility: high` entries — those enter only via blind discovery). Seeds are hypotheses; the steps below confirm or retire them.
  - **Language scoping (non-English books)**: when `BOOK_LANGUAGE` is not English, seed per the catalog's *Language Scope* and *Non-English books* rules — the universal shapes only (drop 29 and the zero-budget exact-string forms of 15, 21, 24, 35, and never seed 28), plus `.authorkit/prompts/_shared/tic-catalog-<lang>.md` when a pack ships for the book's primary subtag (`fr-FR` → `fr`). Record `Lang:` on each seeded entry (`universal` or the subtag). With no pack, seed the universal shapes alone and note in the review that no language-specific string sweep is available yet.
