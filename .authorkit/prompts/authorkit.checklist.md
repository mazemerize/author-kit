...
1. **Setup**: Run `{{SCRIPT_CHECK_PREREQ}}` from repo root and parse JSON for BOOK_DIR and AVAILABLE_DOCS list. All file paths must be absolute.
...
2. **Clarify intent**: Derive up to THREE contextual clarifying questions based on the user's request:
   ...
   If already clear from `{{USER_INPUT_TOKEN}}`, skip questions.

3. **Understand user request**: Combine `{{USER_INPUT_TOKEN}}` + clarifying answers:
