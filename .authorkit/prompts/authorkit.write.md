...
5. **Style anchor refresh** before any drafting / revising / passage help: source style from constitution + the last two approved chapters (`[X]`) before this chapter number. Fallbacks: if one approved chapter exists, use constitution + that chapter; if none, constitution only. Load `.authorkit/templates/style-anchor-template.md` for the canonical section order and headings; write the resolved style anchor to `BOOK_DIR/style-anchor.md` following the template structure exactly.
...
4. **Deliver help by mode** — same shapes as the legacy chapter.help command:
