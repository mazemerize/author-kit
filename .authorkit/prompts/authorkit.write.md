...
1. **Setup**: Run `{{SCRIPT_CHECK_PREREQ}}` from repo root. Parse `BOOK_DIR`, `BOOK_LANGUAGE`, `STYLE_ANCHOR`, and `AVAILABLE_DOCS`. All paths must be absolute. Resolve `BOOK_LANGUAGE` (absent ⇒ `en-US`) to the prose language per the shared guardrails' *Language Protocol* — every draft, plan, outline entry, and world body you write this run is in that language; structural markers, tags, and ids stay English.
...
3. **Always-loaded context**:
   ...
   - The prose language resolved from `BOOK_LANGUAGE` (see the shared guardrails' *Language Protocol*). State it in the opening mode line when it is not English, so the author sees which language this run will produce.
...
Both passes are written in the book's prose language (`BOOK_LANGUAGE`, per the *Language Protocol*) — draft in it directly rather than drafting in English and translating, and follow that language's dialogue punctuation and typographic conventions. Pass B is a voice pass, never a translation pass: it must not change the language of Pass A.
...
3. **Revise pass-by-pass (the Analysis Passes roster).** Revision is explicitly multi-pass: **walk the roster in order** — 1 Style → 2 AI-Tic → 3 In-Chapter Logic → 4 Cross-Chapter/Arc Logic → 5 Disclosure → 6 Standalone → 7 Craft — and for **each pass**:
   ...
   - **Drive gating shapes below budget across the WHOLE draft (Pass 2 only)**: for a shape in the review's **Gating Shapes** set, the fix target is its *whole-draft instance count under the ledger `Budget:`* — not just the quoted spans. The review's citations are a starting point, not the full set: **sweep the entire draft** for the shape yourself, fix enough instances to bring the count below budget, and confirm the post-fix count. For a **zero-budget `phrase`-class shape** (an exact corpus-cliché string, e.g. seed-catalog pattern 29), sweep by **literal search — Grep the whole draft** for the string and its pronoun/tense variants — and clear **every** hit before declaring the finding fixed; the next review's Step B re-greps the same strings, so any survivor re-opens the gate. Fixing only the cited lines is the classic non-convergence trap — blind Step A re-finds the uncited instances next review and the gate never shrinks. "Smallest change" still governs each individual rewrite; it does not license leaving other instances of a *gating* shape in place. Below-budget residual shapes keep the lighter cited-only touch.
...
*Language (see Language Protocol): entity bodies, summaries, and note prose are written in the book's prose language; YAML keys, evolution tags (`(CHxx)`), file names, and entity ids stay English.*
