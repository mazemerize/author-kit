...
3. **Verify draft exists**:
   ...
   - If not: ERROR "Chapter draft not found. Run the `authorkit.chapter.draft` prompt first."
...
8. **Report to user**:
   ...
   - Suggested next action:
     - If PASS: use the `authorkit.chapter.plan` prompt for the next chapter
     - If NEEDS REVISION: re-run the `authorkit.chapter.plan` prompt to re-plan with feedback, then `authorkit.chapter.draft`
