...
The text the user typed is the book description. Assume you always have it available in this conversation. Do not ask the user to repeat it unless they provided an empty input.
...
2. **Check for existing branches before creating new one**:
   ...
   d. Run the script `.authorkit/scripts/powershell/create-new-book.ps1` with the `-Json` flag and the calculated number and short-name:
      - Pass `-Number N+1` and `-ShortName "your-short-name"` along with the book description
      - Example: `.authorkit/scripts/powershell/create-new-book.ps1 -Json -Number 1 -ShortName "victorian-mystery" "A mystery novel set in Victorian London"`
...
3. Load the concept template (see #file:.authorkit/templates/concept-template.md) to understand required sections.
...
7. Report completion with branch name, concept file path, checklist results, and readiness for the next phase (`authorkit.clarify` or `authorkit.outline` prompts).
