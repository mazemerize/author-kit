...
3. **Verify chapter plan exists**:
   ...
   - If not: ERROR "Chapter plan not found. Run the `authorkit.chapter.plan` prompt first."
...
7. **Quality self-check** before saving:
...
8. **Write draft** to `BOOK_DIR/chapters/NN/draft.md`:
...
9. **Update chapter status**: In chapters.md, change this chapter's status from `[P]` to `[D]`:
...
10. **Report completion**:
    ...
    - Suggested next step: use the `authorkit.chapter.review` prompt or the `authorkit.chapter.plan` prompt for the next chapter
