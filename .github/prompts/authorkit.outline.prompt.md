...
1. **Setup**: Run `.authorkit/scripts/powershell/setup-outline.ps1 -Json` from repo root and parse JSON for BOOK_CONCEPT, OUTLINE, BOOK_DIR, CHAPTERS_DIR, BRANCH.

2. **Load context**: Read BOOK_CONCEPT and `.authorkit/memory/constitution.md`. Load OUTLINE template (already copied by script). Also reference the template at #file:.authorkit/templates/outline-template.md for structure guidance.
