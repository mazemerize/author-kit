...
2. **Validate World/ exists**:
   ...
   - If not: ERROR "No World/ folder found. Run the `authorkit.world.build` prompt first."
...
9. **Suggest next actions**:
   - If reciprocal/cross-entity issues: Recommend using the `authorkit.world.build` prompt to add missing entries and fix references
   - If chapter tag integrity issues: Recommend using the `authorkit.world.update` prompt to re-scan
   - If CONCEPT conflicts: Recommend deciding whether CONCEPT or chapter should be authoritative, then updating
   - If staleness detected: Recommend using `authorkit.world.update` for the chapters that were never scanned
