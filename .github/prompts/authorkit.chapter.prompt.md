...
2. **Load chapters.md**: Read `BOOK_DIR/chapters.md` and parse all chapter entries.
   ...
   - If chapters.md doesn't exist: ERROR "No chapters.md found. Run the `authorkit.chapters` prompt first."
...
1. **Load context**:
   ...
   - **Optional**: research.md, `.authorkit/memory/constitution.md`
...
1. **Load context**:
   ...
   - **Required**: concept.md, `.authorkit/memory/constitution.md`
...
- **Maximum 3 revision cycles** per chapter. If the chapter has not passed after 3 draft-review cycles, stop and report:
  ...
  - Recommend manual intervention or running `authorkit.chapter.review` and `authorkit.revise` separately for more targeted fixes.
...
- **Follow each sub-workflow faithfully.** The plan, draft, and review phases follow the same logic as their standalone prompts (`authorkit.chapter.plan`, `authorkit.chapter.draft`, `authorkit.chapter.review`).
