...
**STRICTLY READ-ONLY**: Do **not** modify any files. Assess state and present recommendations only.
...
2. **Load chapters.md**: Read `BOOK_DIR/chapters.md` and parse all chapter entries.
   ...
   - If chapters.md doesn't exist: ERROR "No chapters.md found. Run the `authorkit.chapters` prompt first to generate the chapter task list."
...
- **Surface review feedback.** When the next step is revision, summarize what needs fixing so the user has context.
