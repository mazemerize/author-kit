...
1. **Setup**: Run `.authorkit/scripts/powershell/check-prerequisites.ps1 -Json` from repo root and parse JSON for BOOK_DIR and AVAILABLE_DOCS list. All file paths must be absolute.
...
2. **Clarify intent**: Derive up to THREE contextual clarifying questions based on the user's request:
   ...
   If already clear from user input, skip questions.
...
6. **Structure Reference**: Use the checklist template (see #file:.authorkit/templates/checklist-template.md) for format. Items follow: `- [ ] CHKNNN Description [Category]`

7. **Report**: Output full path to created checklist, item count, and remind user that each run creates a new file.

**Important**: Each invocation of this prompt creates a new checklist file. This allows multiple checklists of different types.
