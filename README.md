...
For cross-model prose continuity, `/authorkit.write` also maintains `book/style-anchor.md`, derived from the constitution plus the *earliest* approved chapters (a fixed origin, so the voice bar doesn't drift downward with recent output).
...
book/
|-- concept.md
|-- style-anchor.md                  # Auto-derived from constitution + earliest approved chapters (fixed origin); managed by write prompt
|-- outline.md
|-- research.md
|-- research/
